...
def test_inflow_past(page):
    ...
    raw_product = product_column.locator("div").first.inner_text().strip()
    product_name = raw_product.split("\n")[0].strip()
...
def test_stock_bulk_edit(page:Page):
    ...
    stock_text1 = stock_column1.inner_text().strip()
    stock_text2 = stock_column2.inner_text().strip()
    product_name1 = product_column1.locator("div").first.inner_text().strip()
    product_name2 = product_column2.locator("div").first.inner_text().strip()
    ...
    inflow_legacy1 = legacy1.inner_text().strip()
    legacy2 = row2.locator("td").nth(6).locator("div")
    inflow_legacy2 = legacy2.inner_text().strip()
    checkbox1.click()
    checkbox2.click()
    page.locator("data-testid=btn_edit_bulk").click()
    page.wait_for_timeout(1000)
