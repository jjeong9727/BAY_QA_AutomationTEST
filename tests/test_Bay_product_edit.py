...
def test_edit_single_product(browser):
    try:
        ...
        # Test result
        msg = f"[PASS][제품관리] 단일 제품 수정 완료: {new_name}"
        save_test_result("test_edit_single_product", msg, status="PASS")
    
    except Exception as e:
        fail_msg = f"[FAIL][제품관리] 단일 제품 수정 실패\n에러 내용: {str(e)}"
        save_test_result("test_edit_single_product", fail_msg, status="FAIL")
        raise
