...
def test_order_status_complete(page: Page):
    status_name = "수령 완료"
    expected = order_status_map["수령 완료"]
    
    try:
        # 제품명 필터링 및 랜덤 선택 후 order_id 가져오기
        filtered_products = filter_products_by_delivery_status(4)  # 필터링된 제품
        if not filtered_products:
            raise ValueError(f"[FAIL] '수령 완료' 상태의 제품이 없습니다.")
        ...
        error_message = f"❌ Error in test_order_status_complete: {str(e)}"
        print(error_message)

        # 실패한 테스트 결과를 저장
        save_test_result("test_order_status_complete", error_message, status="FAIL")
