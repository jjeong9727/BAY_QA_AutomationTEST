...
def test_order_status_fail(page: Page):
    ...
    try:
        ...
        filtered_products = filter_products_by_delivery_status(5)  # 필터링된 제품
        if not filtered_products:
            raise ValueError(f"[FAIL] '발주 실패' 상태의 제품이 없습니다.")
        ...
        error_message = f"❌ Error in test_order_status_발주실패: {str(e)}"
        print(error_message)

        # 실패한 테스트 결과를 저장
        save_test_result("test_order_status_발주실패", error_message, status="FAIL")
